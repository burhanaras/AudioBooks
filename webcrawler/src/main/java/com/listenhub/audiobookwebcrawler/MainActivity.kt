...
class MainActivity : AppCompatActivity() {
        ...
        coroutineScope {
            val deferreds = listOf(
                async {
                    ...
                    val bookz = newReleases.take(2)
                    val chapter0 = Chapter("Chapter 1", newReleases[10].mp3)
                    val chapter1 = Chapter("Chapter 2", newReleases[11].mp3)
                    bookz[0].chapters = mutableListOf(chapter0, chapter1)
                    bookz[1].chapters = mutableListOf(chapter0, chapter1)
                    val json = Gson().toJson(bookz)
                    Log.d(TAG, "$json")
                },
                async {
                ...
                async {
                    var booksOfAllCategories = fetchBooksOfAllCategories()
                }
            )
            deferreds.awaitAll()
    ...
    private suspend fun fetchBooksOfAllCategories(): List<AudioBook> {
        val allBooks = mutableListOf<AudioBook>()
        categories.forEach { category ->
            val url =
                "https://www.audible.com/search?pf_rd_p=daf0f1c8-2865-4989-87fb-15115ba5a6d2&pf_rd_r=3XJ27ZDEFNB6VRNEQD4P&ref=a_search_l1_catRefs_21&node=${category.value}"
            val booksOfThatCategory = fetchBookDetail(url)
            allBooks.addAll(booksOfThatCategory)
            Log.d(TAG, "${category.key} -> ${booksOfThatCategory.size} books")
        }
                    ...
                    parentNode?.let { parentNode ->
                        try {
                            ...
                            val audioBook = AudioBook(
                                id = HashUtils.sha1(name),
                                name = name,
                                author = author,
                                imageUrl = image,
                                mp3 = mp3,
                                description = description,
                                length = len as String,
    ...
    companion object {
        val TAG: String = "MainActivityCrawled"
    }
}
